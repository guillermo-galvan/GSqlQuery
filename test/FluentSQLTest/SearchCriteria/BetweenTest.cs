...
namespace FluentSQLTest.SearchCriteria
{
    public class BetweenTest
    {
        ...
        {
            Between<int> equal = new(_tableAttribute, _columnAttribute, 1);
        ...
        [Theory]
        [InlineData(null, 4, "Test1.Id BETWEEN @Param")]
        [InlineData("AND", 4, "AND Test1.Id BETWEEN @Param")]
        [InlineData("OR", 5, "OR Test1.Id BETWEEN @Param")]
        ...
        public void Should_add_the_Between_query()
        {
            ...
            var andOr = where.Between(x => x.Id, 1);
        ...
        {
            SelectWhere<Test1> where = new(_queryBuilder);
            var andOr = where.Between(x => x.Id, 1).AndBetween(x => x.IsTest, true);
            Assert.NotNull(andOr);
            var result = andOr.BuildCriteria();
            Assert.NotNull(result);
            Assert.NotEmpty(result);
        ...
        {
            SelectWhere<Test1> where = new(_queryBuilder);
            var andOr = where.Between(x => x.Id, 1).OrBetween(x => x.IsTest, true);
            Assert.NotNull(andOr);
            var result = andOr.BuildCriteria();
            Assert.NotNull(result);
            Assert.NotEmpty(result);
    ...
    public class BetweenTest2
    {
        ...
        public void Should_create_an_instance()
        {
            Between2<int> equal = new(_tableAttribute, _columnAttribute, 1,2);
        ...
        {
            Between2<int> equal = new(_tableAttribute, _columnAttribute, initValue, finalValue, logicalOperator);
