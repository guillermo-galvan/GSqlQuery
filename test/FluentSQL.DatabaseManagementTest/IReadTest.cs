...
using FluentSQL.DatabaseManagementTest.Data;
...
namespace FluentSQL.DatabaseManagementTest
{
    public class IReadTest
    {
        ...
        public IReadTest()
        {
            _connectionOptions = new ConnectionOptions<DbConnection>(new FluentSQL.Default.Statements(), LoadFluentOptions.GetDatabaseManagmentMock());
        }

        [Fact]
        public void Retrieve_all_properties_from_the_query2()
        {
            var queryBuilder = DatabaseManagement.IRead<Test1>.Select(_connectionOptions);
        ...
        public void Throw_an_exception_if_null_connectionoptions_is_passed()
        {
            ...
            Assert.Throws<ArgumentNullException>(() => DatabaseManagement.IRead<Test1>.Select(connectionOptions));
        }
        ...
        {
            ConnectionOptions<DbConnection> connectionOptions = null;
            Assert.Throws<ArgumentNullException>(() => DatabaseManagement.IRead<Test1>.Select(connectionOptions, (x) => x.IsTest));
        }
        ...
        {
            ConnectionOptions<DbConnection> connectionOptions = null;
            Assert.Throws<InvalidOperationException>(() => DatabaseManagement.IRead<Test1>.Select(_connectionOptions, x => x));
        }
        ...
        {
            var queryBuilder = DatabaseManagement.IRead<Test1>.Select(connectionOptions);
            Assert.NotNull(queryBuilder);
        ...
        {
            var queryBuilder = DatabaseManagement.IRead<Test1>.Select(connectionOptions, x => new { x.Id, x.Name, x.Create });
            Assert.NotNull(queryBuilder);
            var result = queryBuilder.Build();
            Assert.NotEmpty(result.Text);
        ...
        {
            Assert.Throws<Exception>(() => DatabaseManagement.IRead<Test2>.Select(_connectionOptions, x => x));
        }
        ...
        {
            var queryBuilder = DatabaseManagement.IRead<Test3>.Select(connectionOptions);
            Assert.NotNull(queryBuilder);
            var result = queryBuilder.Build();
            Assert.NotEmpty(result.Text);
            Assert.Equal(query, queryBuilder.Build().Text);
        ...
        {
            var queryBuilder = DatabaseManagement.IRead<Test4>.Select(connectionOptions);
            Assert.NotNull(queryBuilder);
            var result = queryBuilder.Build();
            Assert.NotEmpty(result.Text);
            Assert.Equal(query, queryBuilder.Build().Text);
