...
namespace GSqlQuery.Test
{
    public class IReadTest
    {
        ...
        {
            IQueryBuilderWithWhere<SelectQuery<Test1>, IFormats> queryBuilder = Entity<Test1>.Select(_statements);
        ...
        {
            IQueryBuilderWithWhere<SelectQuery<Test1>, IFormats> queryBuilder = Entity<Test1>.Select(statements);
            Assert.NotNull(queryBuilder);
        ...
        {
            IQueryBuilderWithWhere<SelectQuery<Test1>, IFormats> queryBuilder = Entity<Test1>.Select(statements, x => new { x.Id, x.Name, x.Create });
            Assert.NotNull(queryBuilder);
            var result = queryBuilder.Build();
            Assert.NotEmpty(result.Text);
        ...
        public void Retrieve_all_properties_from_the_query_with_attributes(IFormats statements, string query)
        {
            IJoinQueryBuilder<Test3, SelectQuery<Test3>, IFormats> queryBuilder = Entity<Test3>.Select(statements);
            Assert.NotNull(queryBuilder);
            var result = queryBuilder.Build();
            Assert.NotEmpty(result.Text);
            Assert.Equal(query, queryBuilder.Build().Text);
        ...
        public void Retrieve_all_properties_from_the_query_with_attributes_and_scheme(IFormats statements, string query)
        {
            IJoinQueryBuilder<Test4, SelectQuery<Test4>, IFormats> queryBuilder = Entity<Test4>.Select(statements);
            Assert.NotNull(queryBuilder);
            var result = queryBuilder.Build();
            Assert.NotEmpty(result.Text);
            Assert.Equal(query, queryBuilder.Build().Text);
