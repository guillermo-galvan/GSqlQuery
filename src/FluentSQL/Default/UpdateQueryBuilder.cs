    ...
    internal class UpdateQueryBuilder<T> : QueryBuilderWithCriteria<T, UpdateQuery<T>>, IQueryBuilderWithWhere<T, UpdateQuery<T>> where T : class, new()
    {
        ...
        private List<CriteriaDetail> GetUpdateCliterias()
        {
            ...
            foreach (var item in _columnValues)
            {
                ...
                criteriaDetails.Add(new CriteriaDetail($"{item.Key.GetColumnName(_tableName, _statements)}={paramName}",
        ...
        protected override string GenerateQuery()
        {
            ...
            {
                query = string.Format(_statements.Update, _tableName, string.Join(",", criteria.Select(x => x.QueryPart)));
            }
            else
            {
                string where = GetCriteria();
                query = string.Format(_statements.UpdateWhere, _tableName, string.Join(",", criteria.Select(x => x.QueryPart)), where);
...
#pragma warning restore CS8604 // Possible null reference argument.
        ...
        {
            return new UpdateQuery<T>(GenerateQuery(), _columns, _criteria, _statements);
        }
